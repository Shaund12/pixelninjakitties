...
import { initializeSupabase } from '../public/js/supabase.js';
...
export async function createListing(tokenId, sellerAddress, price, currencyAddress) {
    if (!tokenId || !sellerAddress || !price || !currencyAddress) {
        throw new Error('All listing parameters are required');
    }
    ...
    try {
        ...
        // Log activity
        await logActivity(sellerAddress, 'listing_created', tokenId, { price, currencyAddress });
        return data;
    } catch (error) {
...
export async function cancelListing(tokenId, sellerAddress) {
    if (!tokenId || !sellerAddress) {
        throw new Error('Token ID and seller address are required');
    }
    ...
    try {
        ...
        // Log activity
        await logActivity(sellerAddress, 'listing_cancelled', tokenId);
        return { success: true };
    } catch (error) {
...
export async function completeSale(tokenId, sellerAddress, buyerAddress) {
    if (!tokenId || !sellerAddress || !buyerAddress) {
        throw new Error('Token ID, seller address, and buyer address are required');
    }
    ...
    try {
        ...
        await logActivity(sellerAddress, 'sale_completed', tokenId, { buyerAddress });
        await logActivity(buyerAddress, 'purchase_completed', tokenId, { sellerAddress });
        return { success: true };
    } catch (error) {
...
export async function updateFloorStats(rarity, floorPrice, currencyAddress, sampleSize = 0) {
    if (!rarity || !floorPrice || !currencyAddress) {
        throw new Error('Rarity, floor price, and currency address are required');
    }
    ...
    try {
        ...
        console.error('Error in updateFloorStats:', error);
        throw error;
    }
}
